...
cover: "/blog/flow-operators-cover.png"
featured: false
---
...
This is **Part 3** of a 3-part series on Kotlin for Android developers. If you missed the earlier parts, check out [Part 1: Kotlin Coroutines](/blog/kotlin-coroutines-guide) and [Part 2: Jobs & Cancellation](/blog/kotlin-coroutine-jobs-cancellation) first. In this final part, we go deep on **Kotlin Flow** — the reactive stream API that has completely replaced LiveData in modern Android development.
...
A `suspend` function is "call and get one answer." A [Flow](https://kotlinlang.org/docs/flow.html) is "subscribe and keep getting answers." That single difference changes everything about how you build reactive Android UIs.
...
Google's official recommendation as of 2026: **use [StateFlow](https://developer.android.com/kotlin/flow/stateflow-and-sharedflow) in ViewModels, not LiveData.**
...
Quick reference: `flow {}` and `flowOf()` are cold. [`StateFlow` and `SharedFlow`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines.flow/-state-flow/) are hot.
...
The [`repeatOnLifecycle(STARTED)`](https://developer.android.com/topic/libraries/architecture/coroutines#repeatOnLifecycle) pattern ensures collection stops when the Fragment goes to the background and restarts when it comes back. Never collect Flows directly in `onViewCreated` without this wrapper — you will leak collectors.
