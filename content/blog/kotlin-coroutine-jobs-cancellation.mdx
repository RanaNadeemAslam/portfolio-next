...
cover: "/blog/coroutines-jobs-cover.png"
featured: false
---
...
Every time you call `launch`, it returns a [`Job`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/-job/). Think of it as a **contract for a task you hired someone to do**. You can check on the task, wait for it to finish, or cancel it entirely.
...
[`SupervisorJob`](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines/-supervisor-job.html) changes Rule 3: **a child's failure does NOT cancel the parent or siblings.**
...
This is the part that trips up even experienced developers: **`job.cancel()` doesn't instantly kill a coroutine.** It sets a flag and politely asks the coroutine to stop. The coroutine has to **cooperate** by checking that flag. The [Kotlin cancellation guide](https://kotlinlang.org/docs/cancellation-and-timeouts.html) covers this in detail.
